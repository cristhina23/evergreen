...
{#if open}
	<div class="fixed inset-0 z-999 flex items-center justify-center p-4">
		...
		<div
			...
			class="custom-scrollbar relative z-10 max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-[0_30px_80px_rgba(15,39,68,0.25)] sm:p-8"
		>
			<button
				type="button"
				onclick={closeModal}
				aria-label="Cerrar formulario"
				class="absolute top-5 right-5 flex size-10 items-center justify-center rounded-full bg-(--color-surface) text-(--color-text-secondary) transition hover:bg-(--color-border)"
			>
				<X size={20} />
			</button>
	...
	<div class="py-10 text-center">
		<div
			class="mx-auto flex size-20 items-center justify-center rounded-full bg-[var(--color-secondary-soft)] text-4xl"
		>
			✅
		</div>
		...
		>
			Ya recibimos tu solicitud de información sobre las
			franquicias Evergreen.
...
{:else}
			...
			>
				...
				<button
					type="submit"
					class="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-(--color-secondary) px-6 text-sm font-bold text-white shadow-[0_16px_35px_rgba(76,175,80,0.28)] transition hover:bg-[#43a047]"
				>
